...
void drawPowerFromFronius(){

bool test = gDisplayValues.froniusup ;

        Serial.print("test = gDisplayValues.froniusup : ");
      Serial.println(test);
  ...
  if (test == 1){
    ...
    /*#ifdef  DEVKIT1
      display.setTextAlignment(TEXT_ALIGN_LEFT);
      display.setFont(ArialMT_Plain_10);
      //display.drawString(0, 63,displayedPower);
      //display.drawString(60, 63,generatedPower2);
    #endif */
    //#ifdef  TTGO
      display.setCursor(0, 120, 2); 
      display.setTextColor(TFT_WHITE,TFT_BLACK);  display.setTextSize(1);
      display.print("Prod : ");
      display.println(String(gDisplayValues.production,0) + "W");
      ...
      display.print("conso : ");
      display.println(String(gDisplayValues.production + gDisplayValues.watt,0) + "W");

  }
  else {
      display.setCursor(110, 120, 2);
      display.println("Fronius down");
      }
