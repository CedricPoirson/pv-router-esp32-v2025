  ...
  for (;;){
    ...
    drawtext10(64 ,0, "no Wifi");
      ...
      // gDisplayValues.dimmer = 0 ; /// mise à zero du dimmer par sécurité 
        ...
        if ( gDisplayValues.watt > config.delta ) { drawtext16TTGO(100,70, affiche,TFT_RED );}
        else if ( gDisplayValues.watt < config.deltaneg ) { drawtext16TTGO(100,70, affiche,TFT_BLUE );}
